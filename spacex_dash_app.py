...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df = spacex_df[spacex_df["Launch Site"] == entered_site]["class"].value_counts().to_frame()
        df["name"] = ["Failure", "Success"]
        fig = px.pie(df, values='class', names='name', title='Total Success Launches for ' + entered_site)
        return fig
...
def scatter(input1, input2):
    ...
    if input1 == 'All Sites':
        ...
        df2 = df[ndf["Payload Mass (kg)"] >= input2[0]]
        df3 = df2[df["Payload Mass (kg)"] <= input2[1]]
        fig2 = px.scatter(df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    return fig2
